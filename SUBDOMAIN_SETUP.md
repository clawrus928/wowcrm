# 把 wowcrm 接到 `crm.wowmac.com`

跟 ledger 一台 EC2 / 一個 nginx 共用，獨立 subdomain + port + service。
**Slug**: `wowcrm` ｜ **Subdomain**: `crm.wowmac.com` ｜ **Port**: `127.0.0.1:8002` ｜
...
| Type | Name | Value | TTL |
|---|---|---|---|
| A | `crm` | `13.236.95.177` | 600 |

驗證：

```bash
dig +short crm.wowmac.com
# 應該回 13.236.95.177
```
...
```bash
curl -sI -H "Host: crm.wowmac.com" http://127.0.0.1/
...
```bash
sudo certbot --nginx -d crm.wowmac.com
```
...
```bash
curl -sI https://crm.wowmac.com/
# HTTP/2 200, content-type: text/html

curl -fsS https://crm.wowmac.com/api/health
# {"ok":true,"ts":"..."}

curl -sI http://crm.wowmac.com/
# 301 → https://
...
打開 https://crm.wowmac.com/ → 看到 wowcrm 登入畫面 → 右下角看到對的 git SHA → 完成 ✅
...
- [ ] DNS A record `crm` → `13.236.95.177`，dig 通過
...
- [ ] `curl -H "Host: crm.wowmac.com" http://127.0.0.1/` 200
- [ ] certbot 拿到證書
- [ ] `https://crm.wowmac.com/` 看到登入頁，右下角 SHA 和 master HEAD 對得上
...
# 拿掉憑證
sudo certbot delete --cert-name crm.wowmac.com
